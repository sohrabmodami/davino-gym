...
const socialFields = [
  ...
  { key: 'instagram2', visKey: 'instagram2Visible', label: 'اینستاگرام دوم', placeholder: 'https://instagram.com/davino2...' },
...
function HeroCropModal({ src, onCrop, onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(0,0,0,.65)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
      onClick={onClose}
    >
      <div style={{ background: 'var(--ad-card)', borderRadius: 22, padding: '24px 24px 22px', maxWidth: 520, width: '100%', animation: 'slideUp .25s ease', boxShadow: '0 32px 80px rgba(0,0,0,.35)' }}
        onClick={e => e.stopPropagation()}
      >
        <h3 style={{ fontSize: 16, fontWeight: 900, color: 'var(--text)', marginBottom: 3 }}>برش تصویر هیرو</h3>
        <p style={{ fontSize: 12, color: 'var(--ad-text3)', marginBottom: 16 }}>قاب قرمز را بکش — نسبت ۱۶:۱۰ افقی</p>
  ...
  return (
    <div style={{ padding: '36px 40px', maxWidth: 760 }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 22, fontWeight: 900, color: 'var(--text)', marginBottom: 4 }}>تنظیمات</h1>
        <p style={{ fontSize: 13, color: 'var(--ad-text2)' }}>اطلاعات عمومی و شبکه‌های اجتماعی باشگاه</p>
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            {gymFields.map(({ key, label, placeholder }) => (
              <label key={key} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <span style={{ fontSize: 12, color: 'var(--ad-text2)', fontWeight: 700 }}>{label}</span>
            ...
            {socialFields.map(({ key, visKey, label, placeholder }) => {
              ...
              return (
                <div key={key} style={{
                  ...
                  background: isOn ? 'var(--ad-rowh)' : 'var(--ad-rowh)',
                  border: `1.5px solid ${isOn ? 'var(--ad-card-b)' : 'var(--ad-card-b)'}`,
                  borderRadius: 12, padding: '12px 16px',
                  transition: 'all .2s',
                }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, color: isOn ? 'var(--ad-text2)' : 'var(--ad-text3)', fontWeight: 700, marginBottom: 6 }}>{label}</div>
                    <input
                      ...
                      disabled={!isOn}
                      style={{ background: isOn ? 'var(--ad-card)' : 'var(--ad-rowh)' }}
                    />
                  </div>
        ...
        <div className="settings-section">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginBottom: 24 }}>
            <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <span style={{ fontSize: 12, color: 'var(--ad-text2)', fontWeight: 700 }}>بج (نشان کوچک بالای عنوان)</span>
            ...
            <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <span style={{ fontSize: 12, color: 'var(--ad-text2)', fontWeight: 700 }}>عنوان اصلی (برای شکستن خط از Enter استفاده کن)</span>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, paddingTop: 4 }}>
              ...
              <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <span style={{ fontSize: 12, color: 'var(--ad-text2)', fontWeight: 700 }}>عنوان روی کارت تصویر</span>
          ...
          {/* Hero image */}
          <div style={{ paddingTop: 16, borderTop: '1px solid var(--ad-card-b)' }}>
            <div style={{ fontSize: 12, color: 'var(--ad-text2)', fontWeight: 700, marginBottom: 12 }}>تصویر هیرو</div>
            <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start', flexWrap: 'wrap' }}>
              <div style={{
                width: 240, height: 152, borderRadius: 14, overflow: 'hidden', flexShrink: 0,
                border: '2px dashed var(--ad-card-b)', background: 'var(--ad-rowh)',
                display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative',
              }}>
                {form.heroImage
                  ? <img src={form.heroImage} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  : <div style={{ textAlign: 'center', color: 'var(--ad-text3)' }}>
              ...
              <div>
                <div style={{ fontSize: 13, color: 'var(--ad-text2)', marginBottom: 12, lineHeight: 1.7 }}>
                  پس‌زمینه بخش هیرو سایت.<br/>
                  <span style={{ fontSize: 12, color: 'var(--ad-text3)' }}>پیشنهاد: افقی، حداقل ۱۲۰۰×۶۰۰</span>
                </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, paddingTop: 4 }}>
          {dirty && (
            <button
              type="button"
              onClick={handleReset}
              style={{ background: 'var(--ad-card)', color: 'var(--ad-text2)', border: '1.5px solid var(--ad-card-b)', borderRadius: 12, padding: '11px 24px', fontFamily: 'Vazirmatn', fontSize: 14, cursor: 'pointer', transition: 'all .15s' }}
              onMouseEnter={e => e.currentTarget.style.borderColor = '#bbb'}
              onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--ad-card-b)'}
            >
              بازگشت به قبل
            </button>
          )}
