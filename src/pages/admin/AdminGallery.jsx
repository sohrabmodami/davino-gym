...
function AddModal({ onSave, onClose }) {
  ...
  return (
    <div className="admin-modal-overlay" onClick={onClose}>
      <div style={{ background: 'var(--ad-card)', borderRadius: 20, width: '100%', maxWidth: 460, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 32px 80px rgba(0,0,0,.2)', animation: 'slideUp .25s ease' }} onClick={e => e.stopPropagation()}>
        <div style={{ padding: '22px 26px', borderBottom: '1px solid var(--ad-card-b)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ fontSize: 16, fontWeight: 900, color: 'var(--text)' }}>آیتم جدید گالری</h2>
        ...
        <div style={{ padding: '22px 26px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* Upload zone */}
          <div>
            <span style={{ fontSize: 11, color: 'var(--ad-text2)', fontWeight: 700, display: 'block', marginBottom: 8 }}>تصویر</span>
            {preview ? (
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--ad-text2)', marginBottom: 3 }}>کلیک کن یا عکس رو اینجا بکش</div>
                    <div style={{ fontSize: 11, color: 'var(--ad-text3)' }}>JPG, PNG, WebP — چیدمان خودکار</div>
                  </>
                )}
              </div>
            )}
          ...
          <label style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            <span style={{ fontSize: 11, color: 'var(--ad-text2)', fontWeight: 700 }}>عنوان</span>
            <input className="admin-input" value={form.title} onChange={e => set('title', e.target.value)} placeholder="مثال: دیوار Lead" />
          </label>

          <label style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            <span style={{ fontSize: 11, color: 'var(--ad-text2)', fontWeight: 700 }}>دسته‌بندی</span>
          ...
          <div>
            <span style={{ fontSize: 11, color: 'var(--ad-text2)', fontWeight: 700, display: 'block', marginBottom: 8 }}>چیدمان</span>
            <div style={{ display: 'flex', gap: 8 }}>
              {[['normal', 'عادی (1×1)'], ['wide', 'عریض (2×1)']].map(([val, lbl]) => (
                <button key={val} onClick={() => set('span', val)} style={{
                  ...
                  borderColor: form.span === val ? '#EA443C' : 'var(--ad-card-b)',
                  background: form.span === val ? 'rgba(234,68,60,.06)' : 'var(--ad-rowh)',
                  color: form.span === val ? '#EA443C' : '#aaa',
                  transition: 'all .15s',
                }}>{lbl}</button>
              ))}
            </div>
            <p style={{ fontSize: 11, color: 'var(--ad-text3)', marginTop: 6 }}>بر اساس نسبت تصویر خودکار انتخاب شده — می‌تونی تغییر بدی</p>
          </div>
    ...
    <div style={{ padding: '36px 40px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--ad-text2)' }}>{gallery.length} آیتم — چیدمان خودکار بر اساس نسبت تصویر</p>
        </div>
        <button
      ...
      {gallery.length === 0 && (
        <div style={{ textAlign: 'center', padding: '80px 20px', color: 'var(--ad-text3)' }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--ad-text3)' }}>اولین آیتم را اضافه کنید</p>
        </div>
      )}
    </div>
  )
